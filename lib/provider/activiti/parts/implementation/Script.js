...
module.exports = function(scriptLanguagePropName, scriptValuePropName, isFormatRequired, translate) {
  var uniId = Math.random().toFixed(2);

  return {
    template:
    '<div class="bpp-row bpp-textfield">' +
      '<label for="cam-script-format' + uniId+'">' + escapeHTML(translate('Script Format')) + '</label>' +
      '<div class="bpp-field-wrapper">' +
      //bug2
      '<input id="cam-script-format' + uniId +'" type="text" name="scriptFormat" />' +
    ...
    '<div class="bpp-row">' +
      '<label for="cam-script-type' + uniId +'">' + escapeHTML(translate('Script Type')) + '</label>' +
      '<div class="bpp-field-wrapper">' +
      '<select id="cam-script-type' + uniId +'" name="scriptType" data-value>' +
    ...
    '<div class="bpp-row bpp-textfield">' +
      '<label for="cam-script-resource-val' + uniId +'" data-show="script.isScriptResource">' + escapeHTML(translate('Resource')) + '</label>' +
      '<div class="bpp-field-wrapper" data-show="script.isScriptResource">' +
      //bug1
      '<input id="cam-script-resource-val' + uniId +'" type="text" name="scriptResourceValue" />' +
    ...
    '<div class="bpp-row">' +
      '<label for="cam-script-val' + uniId +'" data-show="script.isScript">' + escapeHTML(translate('Script')) + '</label>' +
      '<div class="bpp-field-wrapper" data-show="script.isScript">' +
      '<textarea id="cam-script-val' + uniId +'" type="text" name="scriptValue"></textarea>' +
      '</div>'+
    '</div>',
